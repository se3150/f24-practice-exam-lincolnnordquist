...
def describe_Battery():
    ...
    def test_initializes_with_capacity():
        b = Battery(100)
        assert b.mCapacity == 100
    
    def test_initializes_with_charge():
        b = Battery(100)
        assert b.mCharge == 100
    
    def test_initializes_with_external_monitor():
        monitor = Mock()
        b = Battery(100, monitor)
        assert b.external_monitor == monitor

    def test_recharges_battery_to_capacity(charged_battery):
        charged_battery.recharge(100)
        assert charged_battery.mCharge == 100

    def test_recharges_battery_partially(partially_charged_battery):
        partially_charged_battery.recharge(20)
        assert partially_charged_battery.mCharge == 90

    def test_does_not_overcharge_battery(charged_battery):
        charged_battery.recharge(10)
        assert charged_battery.mCharge == 100

    def test_drains_battery(partially_charged_battery):
        partially_charged_battery.drain(20)
        assert partially_charged_battery.mCharge == 50

    def test_does_not_drain_below_zero(partially_charged_battery):
        partially_charged_battery.drain(80)
        assert partially_charged_battery.mCharge == 0

    def test_notifies_external_monitor_on_recharge(charged_battery):
        monitor = Mock()
    ...
    def test_allows_stubbing_notify_recharge(charged_battery):
        monitor = Mock()
    ...
    def test_notifies_external_monitor_on_drain(partially_charged_battery):
        monitor = Mock()
    ...
    def test_allows_stubbing_notify_drain(partially_charged_battery):
        monitor = Mock()
